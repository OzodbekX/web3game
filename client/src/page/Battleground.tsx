...
const Battleground = () => {
  ...
  const handleBattleChoice = (ground:any) => {
    setBattleGround&&setBattleGround(ground.id);

    localStorage.setItem('battleground', ground.id);

    setShowAlert&&setShowAlert({ status: true, type: 'info', message: `${ground.name} is battle ready!` });
  ...
  return (
    <div className={`${styles.flexCenter} ${styles.battlegroundContainer}`}>
      {showAlert?.status && <Alert type={showAlert?.type ||"info"} message={showAlert?.message||""} />}
